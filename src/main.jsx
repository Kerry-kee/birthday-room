...
function App(){
  ...
  useEffect(()=>{
    const hideToast=()=>{if(document.hidden){clearTimeout(toastTimer.current);setToast('');}};document.addEventListener('visibilitychange',hideToast);
    ...
    try{if(offline){const probe=document.createElement('canvas'),gl=probe.getContext('webgl2');if(!gl)throw new Error('WebGL2 unavailable');gl.getExtension('WEBGL_lose_context')?.loseContext();}room.current=createRoom(roomHost.current,{onSelect:id=>selectRef.current(id),onReady:()=>setReady(true),onError:sceneError,onMode:setMode,onFocus:setFocus,onLights:setLightsOn,markerElements:markers.current,reducedMotion:reduced.current});}
    catch(e){if(!offline)console.error(e);sceneError('这台设备暂时无法显示 3D 小屋。');}
    return ()=>{document.removeEventListener('visibilitychange',hideToast);room.current?.dispose();audio.current?.dispose();clearTimeout(toastTimer.current);};
  },[]);
  ...
  return <main data-lights-on={String(lightsOn)} data-revealing={String(revealing)} data-cake-opened={String(cakeOpened)} data-music-track={musicTrack} data-music-playing={String(sound)} className={`app-shell immersive ${flatMode?'flat-exploration':''} ${mode.active?'is-exploring':''} ${mode.locked?'mouse-locked':''} ${revealing?'is-revealing':''} ${!lightsOn?'candle-night':''}`}>
    ...
    {flatMode&&<section className="flat-room" aria-label="轻量探索"><h2>小屋里的心意，都还在。</h2><p>已切换为轻量探索。点击物品寻找线索，解开机关，收集礼物。</p><div className="flat-room-grid">{[...extraObjects,...gifts,{id:'lightSwitch',place:lightsOn?'关闭灯光':'打开灯光'},{id:'cake',place:cakeOpened?'生日蛋糕':'生日惊喜礼盒'},{id:'cat',place:'熟睡的小猫'}].map(object=><button key={object.id} onClick={()=>select(object.id)}>{found.includes(object.id)?'✓ ':''}{object.place}</button>)}</div></section>}
