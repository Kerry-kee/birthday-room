...
try{
 ...
 await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x:button.x+button.width/2,y:button.y+button.height/2}]});await page.waitForTimeout(400);await cdp.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});await page.waitForTimeout(150);const moved=await pose();assert(Math.hypot(moved.x-before.x,moved.z-before.z)>.15);
 ...
 await page.getByRole('button',{name:'手记',exact:true}).tap();const journal=page.getByRole('dialog',{name:'探索手记'});await journal.waitFor();const rect=await journal.boundingBox();assert(rect.x>=0&&rect.y>=0&&rect.x+rect.width<=391&&rect.y+rect.height<=845);await page.screenshot({path:'test-results/phone-auto-dialog.png'});
 await page.setViewportSize({width:844,height:390});await page.waitForTimeout(250);assert.equal(await frame.getAttribute('data-landscape-rotated'),'false');await journal.waitFor();await page.getByRole('button',{name:'合上手记，继续探索'}).tap();await page.waitForTimeout(200);assert(!(await pose()).paused);
 const after=await pose();assert(Math.hypot(after.x-looked.x,after.z-looked.z)<.001);assert(!/\bE\b/.test(await page.locator('.interaction-prompt').innerText()));
